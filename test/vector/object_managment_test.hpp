...
/*   Updated: 2022/07/26 14:40:02 by tsiguenz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	objectManagmentTest() {
	...
	}
	{
		...
		assertEqContainer("fill constructor no initialisation", ftV1, stdV1);
		assertEqContainer("Fill Constructor with initialisation", ftV2, stdV2);
		for (int i = 0; i < 5; i++) {
			ftV2[i] += (5 * i);
			stdV2[i] += (5 * i);
		}
